...
#OpenBLAS Compile
#make TARGET=x86 BINARY=32 NOFORTRAN=1 USE_THREAD=0 NUM_THREADS=1 AR=$NDKROOT/toolchains/x86-4.9/prebuilt/darwin-x86_64/i686-linux-android/bin/ar CC="$NDKROOT/toolchains/llvm/prebuilt/darwin-x86_64/bin/clang ${CLANG_FLAGS}" HOSTCC=gcc LDFLAGS=$LDFLAGS
make BINARY=32 NOFORTRAN=1 USE_THREAD=1 NUM_THREADS=1 USE_LOCKING=1 AR=$NDKROOT/toolchains/x86-4.9/prebuilt/darwin-x86_64/i686-linux-android/bin/ar CC="$NDKROOT/toolchains/llvm/prebuilt/darwin-x86_64/bin/clang ${CLANG_FLAGS}" HOSTCC=gcc LDFLAGS=$LDFLAGS
make PREFIX=$PWD/output/i686_multi_thread/ NOFORTRAN=0 install
